...
### Python
    ...
    print(client.models.count_tokens(model="gemini-3.6-flash", contents=prompt))
    # ( e.g., total_tokens: 9 )
...
### Go
    ...
    cache, err := client.Caches.Create(ctx, "gemini-3.6-flash", &genai.CreateCachedContentConfig{
    	Contents: contents,
    })
    if err != nil {
    	log.Fatal(err)
    ...
    prompt := "Please give a short summary of this file."
    countResp, err := client.Models.CountTokens(ctx, "gemini-3.6-flash", []*genai.Content{
    	genai.NewContentFromText(prompt, genai.RoleUser),
    }, nil)
    if err != nil {
    	log.Fatal(err)
    ...
    response, err := client.Models.GenerateContent(ctx, "gemini-3.6-flash", []*genai.Content{
    	genai.NewContentFromText(prompt, genai.RoleUser),
